...
import { updatePortfolio } from "@/app/lib/actions";
...
const UpdatePorfolioForm = ({portfolio}) => {

    const [step, setStep] = useState(1);

    const [images, setImages] = useState([]);
    const [imagesPreview, setImagesPreview] = useState(portfolio.imageUrls);
    ...
    const [data, setData] = useState({
      ...
      otherCategory: portfolio.otherCategory,
  ...
  return (
    ...
    action={async (formData) => {
      ...
      const result = await updatePortfolio(formData, data);
      if (result?.err) {
        //show error
        toast({
          variant: "destructive",
  ...
  >
    <Tabs
      defaultValue={step}
      value={step}
      className="lg:w-2/3 [400px] mx-auto"
    >
        ...
        <Card className="h-ful">
          <div className="grid md:grid-cols-2 px-4 md:px-12 mx-auto col-span-2 py-12 items-start gap-8 t-12">
            ...
            <div className="flex col-span-full justify-center gap-4">
              {step !== 1 && (
                <Button
                  className=" bg-slate-500 hover:bg-slate-600 px-4 font-normal"
                  type="button"
                  onClick={prevStep}
                >
                  Previous
                  ...
                  >
                    {thumbnail ? (
                      ...
                      <Image
                        ...
                        width="300"
                        height="300"
                        className="inner-shadow rounded-[16px] w-full border-2 border-slate-300 aspect-[5/3] object-cover"
                      />
                    )}
                  </label>
                </div>
...
export default UpdatePorfolioForm
